...
\end{itemize}
We have simulated observations in g,r,i bands with the following criteria:  regular cadence of three days (one measurement per band each day of observation shifted by ten minutes between bands), season length equal to 160 days, and \daymax = daymin+40*(1+z) where daymin stands for MJD of the first day of observation, airmass equal to 1.2. Five-sigma depth values considered (24.58,24.22,23.65 for g,r,i bands, respectively) correspond to median values given by kraken\_2026.  Two pairs of (exposure time, number of exposures) were considered: (15,2) and (30,1), corresponding to the snaps/no snaps scenarios.  We have used LSST baseline throughput curves (Fig. \ref{fig:throughput}) to estimate the fluxes measured by the telescope.
...
\end{center}
\end{figure}
Estimated magnitudes are given on Figs. \ref{fig:lcfaint},\ref{fig:lcmedium},\ref{fig:lcbright} for a faint, medium, and bight type Ia supernova, respectively. Minimal magnitudes are summarized in Tab. \ref{tab:minimag} and range from 14-15 (z$\sim$0.01, bright SN) up to $\sim$ 18 (z$\sim$0.05, faint SN). 
